...
# 備考:
#       
def func_cancel_order_all(int_p_no, class_cust_property):
    ...
    str_value = class_cust_property.sSecondPassword     # 第二暗証番号（APIでは省略不可）
    req_item.append(class_req())
    req_item[-1].add_data(str_key, str_value)
...
if bool_login :
    ...
    print('-- 注文一括取消 -------------------------------------------------------------')
    int_p_no = int_p_no + 1
    dic_return = func_cancel_order_all(int_p_no, class_cust_property)
    # 送信項目、戻り値の解説は、マニュアル「立花証券・ｅ支店・ＡＰＩ（ｖ〇）、REQUEST I/F、機能毎引数項目仕様」
